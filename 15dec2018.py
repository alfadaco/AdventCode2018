...
def init_maps(map_literal, elves_power_attak, goblin_power_attak):
    ...
    for i_r in range(0, size[0]):
        ...
        for i_c in range(0, size[1]):
            ...
            if c == 'E':
                ...
                creatures[current_position] = Creature(CreatureType.Elf, elves_power_attak)
            elif c == 'G':
                map[current_position] = '.'
                creatures[current_position] = Creature(CreatureType.Goblin, goblin_power_attak)
            else:
...
def run_turn(map, creatures, current_round, debug):
    death_creatures = []
    for i_r in range(0, map.shape[0]):
        for i_c in range(0, map.shape[1]):
            current_position = (i_r, i_c)
            ...
            neighbour_position = get_adiacent_enemies(map, creatures, current_position,
                                                      creatures[current_position].type)
            ...
            if len(neighbour_position) > 0:
                ...
                if not creatures[weak_neigh].is_alive():
                    ...
                    death_creatures.append(creatures[weak_neigh])
                    ...
                    if sum([c.type == enemy_type for c in creatures[creatures != None]]) == 0:
                        return True, death_creatures

    return False, death_creatures
...
if __name__ == '__main__':
    ...
    for test in tests:
        ...
        elves_attak = 3
        goblin_attak = 3
        map, creatures, map_size = init_maps(test[0], elves_attak, goblin_attak)
        ...
        while not battle_won:
            ...
            battle_won, death_creatures = run_turn(map, creatures, current_round, debug)

            if not first_star:
                death_elves = [c for c in death_creatures if c.type == CreatureType.Elf]
                if len(death_elves) > 0:
                    battle_won = False
                    elves_attak += 1
                    map, creatures, map_size = init_maps(test[0], elves_attak, goblin_attak)
                    current_round = 0

            if current_round > 0 and not battle_won:
        ...
        winner_type = creatures[creatures != None][0].type.value
        score = sum([c.hit_points for c in creatures[creatures != None]])
        last_complete_round = min([c.last_moved for c in creatures[creatures != None]])
        full_score = score * last_complete_round
        if debug:
            print_position(map, creatures)
        print('Elves attak power = ' + str(elves_attak))
        print(winner_type + ' won with a score of ' + str(last_complete_round) + ' * ' + str(score) + ' = ' + str(full_score))

        if expected_score > -1 and first_star:
